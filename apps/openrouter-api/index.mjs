...
const server = http.createServer((req, res) => {
  ...
  if (req.url === "/health" && req.method === "GET") {
    sendJSON(200, { status: "healthy", service: "invoicing" });
    return;
  }
  ...
  if (req.url === "/openrouter/models" && req.method === "GET") {
    try {
            ...
            const openRouterModels = kubeData.items
              ...
              .map((item) => {
                try {
                  ...
                  console.error(
                    `Failed to parse annotation for ${item.metadata.name}`,
                  );
                  return null;
                }
              })
      ...
      k8sReq.on("error", (err) =>
        sendJSON(500, { error: "Request Failed", msg: err.message }),
      );
      k8sReq.end();
    } catch (err) {
