...
resource "ssh_sensitive_resource" "install_k3s_first_master" {
  ...
  commands = [
    ...
    "apt-get update",
    "apt-get install -y curl jq iptables open-iscsi gpg apt-transport-https nfs-common systemd-timesyncd",
...
resource "ssh_sensitive_resource" "install_k3s_other_masters" {
  ...
  commands = [
    ...
    "sysctl -w fs.inotify.max_queued_events=32768",
    "apt-get update",
    "apt-get install -y curl jq iptables open-iscsi nfs-common systemd-timesyncd",
    "systemctl enable --now systemd-timesyncd",
    "echo '${var.nginx_metallb_ip} dex.${var.domain}' | tee -a /etc/hosts",
    "if [ ! -f /usr/local/bin/k3s ]; then curl -sL https://github.com/k3s-io/k3s/raw/main/contrib/util/generate-custom-ca-certs.sh | bash -; fi",
    "curl -sfL https://get.k3s.io | sh -",
