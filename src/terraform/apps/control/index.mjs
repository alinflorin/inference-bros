...
<body>
    ...
    <script type="text/babel">
        ...
        function App() {
            ...
            const apiCall = async (endpoint, setter) => {
                setLoading(true);
                try {
            ...
            // Auto-fetch logic when tabs change (Excluding Usage Stats)
            React.useEffect(() => {
            ...
            const fetchUsage = () => {
                if (!statsKey) {
                    alert("User Key is required for Usage Stats");
                    return;
                }
                apiCall("/usage?key=" + statsKey, setUsageRes);
            };
                        ...
                        {activeTab === 0 && (
                            <Grid container spacing={2} sx={{ height: "100%" }}>
                                <Grid item xs={12} md={3}>
                                    <Stack spacing={2}>
                                        <TextField 
                                            label="Simulation Date (Optional)" 
                                            type="date" 
                                            value={billingDate} 
                                    ...
                                    <div className="table-wrapper">
                                        {invResults ? (
                                                ...
                                                <TableBody>
                                                    {invResults.details.map((row, i) => (
                                                        <TableRow key={i} hover>
                                                            <TableCell sx={{ fontWeight: 'bold' }}>{row.invoice.customer_name}</TableCell>
                                                            <TableCell>€{row.invoice.total_cost.toFixed(2)}</TableCell>
                                                            <TableCell>
                                                                <Chip 
                                                                    label={row.sync_result.status} 
                                                                    size="small" 
                                                                    color={row.sync_result.status === "success" ? "success" : "default"} 
                                                                />
                                                            </TableCell>
                                                            <TableCell sx={{ fontSize: "10px", color: "#888" }}>
                                                                {row.sync_result.odoo_id || row.sync_result.error || "-"}
                                                            </TableCell>
                                                        </TableRow>
                                                    ))}
                                                </TableBody>
                                            ...
                                            <Box sx={{ p: 4, textAlign: "center", color: "#aaa" }}>
                                                Configure parameters and run invoicing to see results.
                                            </Box>
                                        )}
                                    </div>
                                </Grid>
                        ...
                        {activeTab === 2 && (
                            <Stack spacing={2} sx={{ height: "100%" }}>
                                <Stack direction="row" spacing={2} alignItems="center">
                                    <TextField 
                                        label="User Key (Required)" 
                                        size="small" 
                                        value={statsKey} 
                                        onChange={e => setStatsKey(e.target.value)} 
                                        sx={{ width: 350 }}
                                    />
                                    <Button variant="contained" onClick={fetchUsage} disabled={loading} disableElevation>
                                        {loading ? <CircularProgress size={20} color="inherit" /> : "Fetch Usage Stats"}
                                    </Button>
                                </Stack>
                                <pre className="json-display">{usageRes ? JSON.stringify(usageRes, null, 2) : "// Enter user key and click fetch"}</pre>
                            </Stack>
                        )}
